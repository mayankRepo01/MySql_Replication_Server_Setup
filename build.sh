...
docker exec mysql_master sh -c 'chmod 600 /etc/mysql/conf.d/mysql.cnf'
docker exec mysql_master sh -c 'rm -rf var/lib/mysql/auto.cnf'
...
docker exec mysql_slave sh -c 'chmod 600 /etc/mysql/conf.d/mysql.cnf'
docker exec mysql_slave sh -c 'rm -rf var/lib/mysql/auto.cnf'
...
docker-compose exec mysql_slave sh -c 'export MYSQL_PWD=111; mysql -u root -e "SET GLOBAL server_id = 2;"'
